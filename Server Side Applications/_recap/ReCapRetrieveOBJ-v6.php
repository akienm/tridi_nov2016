...
/*
 ...
 3D Model.    When it is launched, it is passed the ScansetID of the original scan
 folder.   It uses this ScansetID  to determine what was the photosceneID of the model.
 This is done by looking at the contents of a file with that ScansetID filename in 
 	http://test.soundfit.me/Scans/b1/maps/$scansetID
 Next, this program will create a folder with the name of the ScanID in the
  	http://test.soundfit.me/Scans/b1/modeled/$scanID/$scansetID/
...
// ----------------  GET SCANSETID AND PHOTOSCENE ID ----------------------------
// This program is called with a URL that defines the scansetID.
// if this program is run from the command line, the scansetID is passed as a CLI argument.
if (php_sapi_name() === 'cli') {parse_str(implode('&', array_slice($argv, 1)), $_GET);}
echo "ReCapRetrieveOBJ-v6.php \n";
echo "parameters = ".print_r($_GET, true); 
$scansetID = htmlspecialchars($_GET["scansetID"]);
echo "Retrieving scansetID = $scansetID\n";
$scanID = substr($scansetID,0, strlen($scansetID) -1 ); 

// Once we know the scansetID, we can check the maps directory that has mappings between 
// scanIDs and photoSceneIDs.   We need the photoSceneID to retrieve the proper OBJ file.

$mapfile = $mapdir . $scansetID;
...
// ----------------  END OF GET SCANSETID AND PHOTOSCENE ID ---------------------
...
if (file_exists($emailmapfile)) {
    $xml = simplexml_load_file($emailmapfile);
 	...
 	$from = $xml->recipient;
	echo "to:   $to\n";
	$output_string = $output_string . "to:   $to\n";
	echo "from: $from\n";
	$output_string = $output_string . "from: $from\n";
...
/*
             ...
             $subject = "LAUNCHED: ScansetID($scansetID) / model($photoSceneID)";
             "ReCapRetrieveOBJ.php Callback function launched -- scansetID = $scansetID";
...
echo "\nRetrieving PhotoSceneID $photoSceneID for ScansetID $scansetID\n\n";
...
if ($xml->Error) { // if error stop here else continue
	...
	echo "photoscene $photoSceneID completed by Recap in  $processingTime seconds.\n"; 
	$output_string = $output_string . "photoscene $photoSceneID completed by Recap in  $processingTime seconds.\n";  
}
...
if ($xml->Error) { // if error stop here else continue
	...
	echo "OBJ model for ScansetID: $scansetID \nis at: $objURL\n\n";
	$output_string = $output_string . "OBJ model for ScansetID: $scansetID \nis at: $objURL\n\n";
	...
	$scanset_directory = $modeled_dir_root.$scanID/$scanset;
	if (!file_exists($scanset_directory)) {$retval = mkdir($scanset_directory,0775);}
	...
	$scan_zipfile = $scanset_directory . '/' . $scansetID . ".obj.zip";
	set_time_limit(0); 
	...
	if ($res === TRUE) {  // If download was successful, Unzip and convert 
  		...
  		$infilename =  $scanset_directory . '/' .  "mesh.obj";
  		// $tempfilename = "./tmp/SCS" . date("U");
  		$tempfilename = $scanset_directory . '/' .  "mesh";
  		...
  		echo "SUCCESS: $scansetID OBJ and STL models now in $ModeledURL$scanID/\n\n";
  		$output_string = $output_string . "SUCCESS: $scansetID OBJ and STL models now in $ModeledURL$scanID/\n\n";
  		$subject = "SUCCESS: ScansetID($scansetID) / model($photoSceneID) is ready for manufacturing";
  		$message =  "$subject\n"
  			. "New order: $ReceivedURL$scanID/$neworderfile received.\n"
  			. "OBJ Directory: $ModeledURL$scanID/$scansetID/\n"
  			. "STL: $ModeledURL$scanID/$scansetID/mesh.stl\n"
  			. "Scan images in $ReceivedURL$scanID. \n\n"
  			. $output_string;
