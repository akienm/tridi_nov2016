...
echo "<pre>";    
echo "\n------- ReCapLaunchModeling-wScale+Crop9e.php ------------------------------\n\n";
$outputString = "Scan processing by ReCapLaunchModeling-wScale+Crop9e.php.\n\n";
...
/*
 ...
 Revision 9e  --  This version will use a manual template file if one is specified, 
 or will use an automated one if it is not.    In either case the automated template is
 created, as it can be used to compare the resulting camera positions to the expected
 positions.
 
 This version creates supports an offset to the elevation in automatic 
 template generation to deal with the fact that the camera arm is bent upward so that 
 where the camera is pointed is ABOVE the origin, so the R(x) needs to be less than 
 the elevation.
 
 This is an INTERIM version, that hard codes the R(x) offset
...
define ('CALLBACKPGM', 'http://test.soundfit.me/apps/_recap/ReCapRetrieveOBJ-v9e.php?scanID=');
...
// MANIFEST: get the <Zstring> fields from the Manifest file into an XML string.
$zstring 	= 	$manifestXML -> zstring;
$zstringXMLString =  $manifestXML -> zstring ->asXML();

// Right now the one parameter we expect to get from the ZString is the RXOffset
// Manifest tag <RX> inside <Zstring>.
// If the RX parameter is set viea the command line or query string, it over-rides
...
$to = htmlspecialchars($_GET["RX"]);  // RX is the RxOffset 
if ($RxOffset == NULL) {
	$RxOffset 	=  $manifestXML -> zstring -> rx;
...
if ($attachmentsString == NULL) {
	...
	$outputString = $outputString . "Number of image files = $numberOfAttachmentFiles\n";
...
$xml =$response->xml () ;
echo "xml response =\n";  echo var_dump($xml); echo "\n\n"; // need to understand what comes back
...
foreach ($filesobj as $filesobjkey => $filesobjvalue) 
{
	...
	// echo " filehashID[$filecount] = $hashIDwithSlashes ";  // uncomment to check renaming
...
echo "filehashID =\n"; echo var_dump($filehashID);  echo "\n\n";
echo "filenames =\n"; echo var_dump($filenames);  echo "\n\n";
...
for ($i = 0; $i < $numberOfImageFiles; $i++) // use for production
{
	...
	$templateRotation = -$rotation[$i];
	if ($templateRotation <= -180 ) {
		$templateRotation = 360 + $templateRotation;
		$R->addAttribute('z', "$templateRotation");
	} else if ($templateRotation > 180  ) {
		$templateRotation = $templateRotation - 360;
		$R->addAttribute('z', "$templateRotation");		
...
// ----------------- END Write data to database ------------------------------------------	
	 ...
	 echo "\n-------- END OF ReCapLaunchModeling-wScale+Crop9e.php. ---------------\n\n";
