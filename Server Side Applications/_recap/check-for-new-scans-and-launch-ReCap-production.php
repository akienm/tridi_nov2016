...
echo "\n------- check-for-new-scans-and-launch-ReCap.php -----------------------------------\n\n";
		...
		if (is_dir($NewDir.$entry)) {  // we only process directories
			...
			if ($entry != "." && $entry != "..") {
				...
				if (count($orderfile) > 0) {
					...
					$shellcmd =  "/usr/local/php53/bin/php /home/earchivevps/test.soundfit.me/apps/_recap/ReCapLaunchModeling-wScale+Crop4.php scanID=" 
					  			.  $entry;
